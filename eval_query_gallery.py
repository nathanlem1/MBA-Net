...
def load_network(network):
    save_path = os.path.join(f_name, m_name, 'net_%s.pth' % opt.which_epoch)  # Make sure which model to use!
    ...
    for data in data_loaders:
        ...
        for i in range(2):
            ...
            if opt.part_h * opt.part_v > 1 and opt.use_attention:
                ...
                output_ffs = torch.cat((output_ffs0, output_ffs[1], output_ffs[2], output_ffs[3]), 1)  # For using all
                # components
...
def get_id(img_path):
    ...
    for path, v in img_path:
        ...
        label = path.split('/')[-2]  # For 11k hand data set
...
for i in range(len(galleries)):
    ...
    # Extract feature
    with torch.no_grad():
        gallery_feature = extract_feature(model, data_loaders[g])
        query_feature = extract_feature(model, data_loaders[q])
